...
- Добавлен браузерный GeoIP, который позволяет определять город местоположения пользователя по его IP-адресу. 
  ...
  ```
  Для проверки мы рекомендуем использовать базу [db-ip "IP to City lite"](https://db-ip.com/db/lite.php). Эта бесплатная база имеет довольно точную выборку для российских IP, но названия городов на английском.
