...
- **`v4-7637`** 🟡 Проверка SSL-сертификатов: в синтетическом мониторинге появилась страница «Сертификаты» — список отслеживаемых узлов со сроком действия, издателем и SAN, сводка по статусам и график ошибок по дням. Проверки выполняет отдельный чекер рядом с инфраструктурным агентом внутри изолированного сегмента: он сам забирает свой список целей у Collector и присылает результаты, входящие подключения в сегмент не нужны. Включается переменной `SYNTHETICS_SSL_ENABLED`.
- **`v4-7632`** 🟡 Kubernetes: новый раздел мониторинга кластеров. Список кластеров показывает версию, число узлов и неготовых узлов, namespace'ы, нагрузки и поды, утилизацию CPU и памяти и долю заказанных ресурсов; со страницы кластера можно провалиться в узлы, namespace'ы и контейнеры с их CPU, памятью и перезапусками. Данные собирает инфраструктурный агент, развёрнутый в кластере.
...
- **`v4-7606`** 🎨 Метки, заданные на странице управления метками (появилась в `v4-7558`), теперь работают в фильтре по окружению: достаточно проставить хосту метку `environment` в интерфейсе — главная страница и страница хостов сразу её учитывают, править конфиг агента и перезапускать его не нужно.
- **`v4-7596`** 🔧 Ссылки на страницах хостов, контейнеров, NGINX и баз данных ведут внутрь интерфейса GMONIT, а не на старые дашборды.
- **`v4-7595`** 🔧 Карта сервисов: в подсказке связи основная метрика подписана как RPM вместо служебного имени.
- **`v4-7594`** 🔧 Мобильный мониторинг: исправлена панель «пользователи без падений» на странице версий.
...
- **`v4-7575`** 🟡 MCP: всё, что делается в интерфейсе GMONIT, теперь доступно и AI-агенту — 39 инструментов по всем модулям: APM и транзакции, инфраструктура и серверы баз данных, логи, браузерный и мобильный мониторинг, 1С, инциденты, цели уровня сервиса и бизнес-процессы. Агент не только отвечает на вопросы, но и действует: создаёт цели уровня сервиса с проверкой на текущих данных, правит пороги оповещений, запрашивает разбор первопричины инцидента, — а в клиенте появляются готовые сценарии расследования (`/gmonit:rca`, `/gmonit:triage`, `/gmonit:health-report`), которые не нужно устанавливать отдельно.
- **`v4-7558`** 🔧 Инфраструктура: исправлено сохранение данных инфраструктурного агента.
- **`v4-7558`** 🔧 Менеджер инцидентов: исправлена пометка ключевых метрик в выбросах — отклонения по HttpDispatcher, WebTransaction, ошибкам и Apdex снова попадают в фильтр значимых.
...
- **`v4-7558`** 🔧 Исправлен фильтр окружения на страницах хостов и главной: хосты без метки окружения больше не попадали в выборку по конкретному окружению.
...
- **`v4-7558`** 🟡 Метки (labels): в настройках появилась страница управления метками для APM-, браузерных и мобильных приложений и хостов. Метки проставляются на входящие данные и доступны при фильтрации и в оповещениях.

### Июль 2026

- **`v4-7495`** 🔧 1С: исправлены панели «подозреваемых» в техническом журнале.
...
- **`v4-7462`** 🎨 Менеджер инцидентов: оповещения с меткой `gmonit_incident_allowed` показываются в инциденте независимо от выбранного фильтра.
- **`v4-7460`** 🎨 Менеджер инцидентов: обновлён дизайн — переработаны навигация, страница инцидента и панель пиковых метрик; инциденту теперь можно задать собственное имя.
...
- **`v4-7411`** 🎨 Главная страница: переработана таблица оповещений — детали оповещения отображаются нагляднее.
...
- **`v4-7353`** 🚀 Таблицы инфраструктурного мониторинга переведены на партиционирование — запросы и очистка данных по TTL стали быстрее.
...
- **`v4-7311`** 🚀 Менеджер инцидентов: ускорены запросы к выбросам на странице инцидента и на таймлайне assertions.
