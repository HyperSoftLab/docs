...
- **`v4-6528`** ✨ Восстановлена поддержка закреплённых дашбордов на страницах APM и Browser (теги `custom_nr_apm_app` / `custom_nr_browser_app`).
- **`v4-6523`** 🔧 Исправления в карточке инцидента: корректная фильтрация ML-корреляций и обработка диапазона времени.
