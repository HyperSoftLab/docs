...
1. обновите GMonit до `v3-4470`
2. обновите Clickhouse до 25.3
3. обновите Redis до 7.2
4. закомментируйте переменную окружения `DATOMIC_URI` у контейнера collector
5. проверьте, что collector не использует переменную окружения `BROWSER_MONTINORING_ENABLED`, содержащую опечатку
6. задайте контейнеру grafana переменную оружения `GMONIT_LICENSE`, по аналогии с контейнером collector
7. остановите и удалите контейнер transactor
8. перезапустите контейнеры
9. обновите GMonit до `v4-xxxx`
