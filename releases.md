...
- **`v4-7637`** 🟡 Проверка SSL-сертификатов: в синтетическом мониторинге появилась страница «Сертификаты» — срок действия, издатель и SAN по каждому отслеживаемому узлу, сводка по статусам и график ошибок по дням. Сертификаты в изолированном сегменте проверяются без входящих подключений в него: чекер ставится рядом с инфраструктурным агентом и сам забирает список целей у Collector. Включается переменной `SYNTHETICS_SSL_ENABLED`.
- **`v4-7632`** 🟡 Kubernetes: новый раздел мониторинга кластеров — состояние узлов, namespace'ов, нагрузок и подов с утилизацией CPU и памяти и долей заказанных ресурсов, с проваливанием до контейнера и его перезапусков. Отдельный агент разворачивать не нужно: данные собирает инфраструктурный агент в кластере.
...
- **`v4-7576`** 🚀 Браузерный мониторинг: загрузчик и чанки агента отдаются самой Grafana — RUM работает в закрытом контуре без доступа к внешним CDN.
