...
- **`v4-6953`** 🔧 Incidents: фильтр окружения теперь применяется и к таблице инцидентов.
- **`v4-6950`** 🔧 Incidents: правки отрисовки в панели «Оповещения и аномалии».
...
- **`v4-6890`** 🔧 Incidents: хотфиксы конфигурации ML-сервиса и фильтра окружения.
...
- **`v4-6865`** 🔧 Incidents: при ошибках конфигурации ML-сервиса показывается понятное сообщение вместо общего сбоя.
- **`v4-6857`** ✨ Incidents: фильтр окружения поддерживает выбор сразу нескольких значений.
...
- **`v4-6832`** ✨ Incidents: добавлен фильтр по окружению на таймлайне и панели выбросов.
- **`v4-6828`** 🆕 На главную добавлен список активных оповещений — теперь его видно сразу при заходе, без перехода в Alerting Grafana.
- **`v4-6822`** 🔧 Incidents: исправлен errors-barchart в карточке инцидента.
...
- **`v4-6772`** ✨ Incidents: расширения панели «Оповещения и аномалии» в карточке инцидента.
...
- **`v4-6738`** ✨ Business Monitoring: следующая итерация — обновлённая конфигурация бизнес-системы и страница карты процессов.
- **`v4-6730`** ✨ Incidents: в карточку инцидента добавлена панель «Оповещения и аномалии».
