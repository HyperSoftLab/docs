...
- **`v4-7094`** ✨ Менеджер инцидентов: на таймлайне аномалий плотные группы выбросов теперь раскрываются по клику — можно посмотреть каждый отдельный интервал отклонения, не теряя обзора.
