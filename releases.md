...
- **`v4-7248`** 🔜 MCP-сервер: интегрирован skill — встроенные плейбуки расследования (RCA по алерту и симптому), справочник таблиц, UDF и шаблонные SQL-запросы. AI-агенты теперь работают с GMonit предсказуемее и без выдумывания структур.
...
- **`v4-7206`** 🔜 Релиз коробочных алертов GMONIT и страницы их настройки. Алерты поставляются с шаблонами и порогами по умолчанию; пороги переопределяются прямо из интерфейса — на уровне приложения, хоста, контейнера или БД. Поддержана эскалация по матрице RACI — для каждого правила задаются ответственные роли и уровни оповещения. Включение и интервал срабатывания управляются переменными окружения `GMONIT_GRAFANA_DEFAULT_ALERT_PACK_ENABLED` (по умолчанию `false`) и `GMONIT_GRAFANA_DEFAULT_ALERT_PACK_INTERVAL` (по умолчанию `1m`).
