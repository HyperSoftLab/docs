...
- **`v4-7756`** 🟡 SLO: цель уровня сервиса можно построить на собственном оповещении — форма берёт запрос правила, спрашивает оператор, порог и колонки времени и значения, и считает по ним SLI с бюджетом ошибок и burn rate.
- **`v4-7756`** 🔧 SLO: цели с типом «SQL-условие» снова открываются — раньше просмотр такой цели не работал.
- **`v4-7748`** 🟡 Трассировки: поиск по идентификатору трассировки. Если трейс лежит вне выбранного интервала — появляется подсказка с кнопкой перехода к его окну; если такого трейса нет — так и сказано.
- **`v4-7746`** 🔧 Логотип и переход «домой» в оболочке Grafana ведут на главную GMONIT, а не на домашний дашборд Grafana.
- **`v4-7743`** 🔧 Координация коллекторов через ClickHouse Keeper: после переподключения сессия считается истёкшей, как это и есть на самом деле, — распределённые блокировки больше не продолжают работу на потерянной сессии.
- **`v4-7731`** 🟡 Базовая версия Grafana обновлена до 12.4.9.
- **`v4-7731`** 🔧 Кастомные дашборды: аннотации, скрытые в настройках дашборда, больше не рисуются на панелях.
- **`v4-7725`** 🎨 Настройки вынесены в отдельный раздел со своей навигацией по страницам (агенты, хранение данных, метки, правила транзакций) и обзорной страницей: установленная версия, автор, дата сборки и состав поставки.
- **`v4-7723`** 🟡 MCP: сценарий `gmonit:cio-review` — обзор установки за период для руководителя: вердикт, где пострадал бизнес, сравнение с прошлым периодом и кому что исправлять.
- **`v4-7718`** 🔧 Метрики OpenTelemetry: квантили и средние по гистограммам считаются по середине бакета (для экспоненциальных — по геометрической), а не по его границе — значения больше не смещены вверх.
- **`v4-7715`** 🟡 MCP: добавлено чтение Kubernetes — узлы, пространства имён, рабочие нагрузки, поды и контейнеры с перезапусками. Сценарии расследования принимают параметр `audience`: ответ пишется либо для инженера, который будет действовать, либо для того, кто принимает решение; без параметра агент сам спросит, для кого пишет.
- **`v4-7714`** 🔧 Kubernetes: колонка использования диска на узлах больше не показывает ноль.
- **`v4-7708`** 🎨 Профилирование: у панели результата появилось описание — как читать flame graph и что означают колонки self и total.
- **`v4-7688`** 🔧 Геолокация по IP: значение, которое не является IP-адресом, больше не уходит на разрешение имени — Collector не подвисает на DNS-запросе и не пишет в журнал `UnknownHostException`.
- **`v4-7677`** 🔧 Синтетический мониторинг: недоступность считается по факту — ошибка соединения (кода ответа нет) идёт в отказ, клиентские 4xx — нет, а интервалы, когда чекер молчал, больше не записываются цели в простой.
- **`v4-7677`** 🔧 APM: UUID в пути URL заменяется на `*`, даже если начинается с буквы — транзакции с идентификатором в пути больше не размножаются на отдельные записи.
- **`v4-7652`** 🎨 Фильтр окружения сохраняется при переходе между разделами: выбранное окружение больше не сбрасывается на страницах приложений, хостов, контейнеров, инцидентов и карты сервисов.
- **`v4-7647`** 🟡 Kubernetes: новый раздел мониторинга кластеров — состояние узлов, пространств имён, рабочих нагрузок и подов с утилизацией CPU и памяти и долей заказанных ресурсов, с проваливанием до контейнера и его перезапусков. Отдельный агент разворачивать не нужно: данные собирает инфраструктурный агент в кластере.
- **`v4-7645`** 🟡 Проверка SSL-сертификатов: в синтетическом мониторинге появилась страница «Сертификаты» — срок действия, издатель и SAN по каждому отслеживаемому узлу, сводка по статусам и график ошибок по дням. Сертификаты в изолированном сегменте проверяются без входящих подключений в него: чекер ставится рядом с инфраструктурным агентом и сам забирает список целей у Collector. Включается переменной `SYNTHETICS_SSL_ENABLED`, чекерам дополнительно нужен общий токен. [Документация](https://docs.gmonit.ru/#/modules/synthetics/setup?id=Включение-проверки-сертификатов).
- **`v4-7637`** 🟡 Бизнес-мониторинг: новый модуль. Показывает сквозную конверсию по бизнес-процессу — сколько экземпляров дошло до конца, на каком шаге они отваливаются, по какой причине и сколько занимает прохождение. Шаги описываются условиями над уже собираемой телеметрией, браузерными событиями и трейсами бэкенда, поэтому отдельный агент не нужен и доработка приложения не требуется. Сквозной идентификатор процесса тоже не нужен: соседние шаги связываются по тем полям, которые у них уже общие, и цепочка собирается по частям. Предрасчёта нет — показатели считаются по сырым событиям в момент запроса, поэтому данные видны сразу по поступлении, без задержки на агрегацию. В разделе: воронка конверсии со сравнением с предыдущим периодом, операционный контроль зависших экземпляров по шагам, разбор причин незавершённых сессий, время прохождения в разрезе шагов, карта процесса и путь отдельного экземпляра по событиям. Определения бизнес-систем и процессов задаются прямо в интерфейсе. Модуль включается переменной окружения GMONIT Grafana `GMONIT_BUSINESS_MONITORING_ENABLED`.
- **`v4-7632`** 🔧 Менеджер инцидентов: длительность инцидента в списке больше не занижается — она считается по всем сигналам, включая выбросы за границами оповещений; исправлены ошибки при открытии инцидентов с выбросами и фильтр по окружению.
- **`v4-7632`** 🔧 Кастомные дашборды: исправлены ссылки и повторяющиеся (repeat) панели и ряды — значения переменной подставляются корректно.
- **`v4-7632`** 🎨 SLO: панель со страницы SLO можно скопировать в свой дашборд — в копию попадают все запросы панели, включая скрытые вычисления бюджета ошибок и burn rate.
- **`v4-7606`** 🎨 Лейблы, заданные на странице управления метками (появилась в `v4-7558`), теперь работают в фильтре по окружению: достаточно проставить хосту метку `environment` в интерфейсе — главная страница и страница хостов сразу её учитывают, править конфиг агента и перезапускать его не нужно.
- **`v4-7596`** 🔧 Ссылки на страницах хостов, контейнеров, NGINX и баз данных ведут внутрь интерфейса GMONIT, а не на старые дашборды.
- **`v4-7584`** 🟡 Настройки хранения данных: сроки хранения браузерных событий и RUM-логов теперь задаются в интерфейсе, список категорий дополнен и приведён в порядок (`BROWSER_INSIGHTS_TTL_DAY` и др.).
- **`v4-7580`** 🎨 1С: в техническом журнале появился фильтр по сеансу и подробности события в выдвижной панели, уточнены панели памяти и процессорного времени в сеансах, переупорядочено меню раздела.
- **`v4-7575`** 🟡 MCP: покрытие расширено на APM и транзакции, инфраструктуру и серверы баз данных, логи, браузерный и мобильный мониторинг, 1С, инциденты, цели уровня сервиса и бизнес-процессы. Помимо чтения агенту доступны изменяющие действия: создать цель уровня сервиса с проверкой на текущих данных, поправить порог оповещения, запросить разбор первопричины инцидента. В клиенте доступны готовые сценарии расследования (`/gmonit:rca`, `/gmonit:triage`, `/gmonit:health-report`), устанавливать их отдельно не нужно.
- **`v4-7558`** 🔧 Менеджер инцидентов: отклонения по времени отклика, пропускной способности, ошибкам и Apdex снова помечаются как значимые и попадают в соответствующий фильтр выбросов.
- **`v4-7558`** 🟡 Менеджер инцидентов: фильтры оповещений и выбросов по умолчанию задаются переменными `GMONIT_INCIDENT_ALERT_FILTER_DEFAULTS` (по умолчанию `gmonit-apm`) и `GMONIT_INCIDENT_OUTLIERS_FILTER_DEFAULTS` (по умолчанию `red`).
- **`v4-7558`** 🔧 Исправлен фильтр окружения на страницах хостов и главной: хосты без лейбла окружения больше не попадали в выборку по конкретному окружению.
- **`v4-7558`** 🎨 Хосты: в таблицу добавлена колонка версии инфраструктурного агента — сразу видно, где агент устарел.
- **`v4-7558`** 🟡 Настройки агентов: Apdex T задаётся прямо в интерфейсе, с подсказкой на основе фактического p90 времени отклика приложения — порог больше не нужно править в конфиге агента и перезапускать его.
- **`v4-7558`** 🟡 Лейблы: в настройках появилась страница управления лейблами для APM-, браузерных и мобильных приложений и хостов. Лейблы проставляются на входящие данные и доступны при фильтрации и в оповещениях.

### Июль 2026

- **`v4-7495`** 🔧 1С: исправлены панели вероятных причин таймаутов в техническом журнале.
- **`v4-7490`** 🟡 Подтверждение оповещений: на новой странице видны все горящие оповещения, и любое из них можно подтвердить на заданный срок — с фиксацией автора, времени и журналом подтверждений. Подтверждение останавливает дальнейшую эскалацию по матрице RACI: пока дежурный разбирается, оповещение не уходит на следующий уровень ответственных.
- **`v4-7489`** 🟡 Маршрутизация оповещений вынесена в отдельную страницу: правила направляют оповещения получателям по уровням эскалации L1/L2/L3, а страница переопределений отвечает теперь только за пороги.
- **`v4-7489`** 🟡 Коробочные оповещения: отсутствие данных считается падением приложения только тогда, когда сам Collector присылает метрики — при недоступности Collector лавина ложных оповещений «сервис недоступен» больше не возникает. Параметры пакета (`GMONIT_COLLECTOR_APP_NAME`, `GMONIT_COLLECTOR_SELF_MONITORING_ENABLED`) применяются при старте контейнера, без пересборки образа. [Документация](https://docs.gmonit.ru/#/modules/alert-pack/setup?id=Подавление-ложных-оповещений-«нет-данных»-при-недоступности-collector).
- **`v4-7460`** 🎨 Менеджер инцидентов: инциденту теперь можно задать собственное имя вместо сгенерированного; обновлены навигация, страница инцидента и панель пиковых метрик.
- **`v4-7458`** 🟡 MCP: аудит действий. Обращения к MCP (вход, вызовы инструментов, выгрузки данных с текстом запроса, ошибки) теперь можно писать в отдельный структурированный журнал (файл или STDOUT) и передавать в SIEM. [Документация](https://docs.gmonit.ru/#/modules/mcp/setup?id=Аудит-событий).
- **`v4-7455`** 🟡 MCP: персональная аутентификация и роли. Клиенты теперь могут подключаться по личным токенам сервис-аккаунтов Grafana, а доступ к инструментам ограничивается ролью (Viewer/Editor/Admin) — вместо единого общего токена на всех. [Документация](https://docs.gmonit.ru/#/modules/mcp/setup?id=Аутентификация-по-токену-grafana).
- **`v4-7423`** 🎨 Менеджер инцидентов: добавлена фильтрация оповещений и выбросов и группировка по связанности сервисов.
- **`v4-7411`** 🎨 Главная страница: в таблице оповещений одно и то же оповещение больше не размножается по строкам на каждый уровень эскалации — уровни с длительностью и началом, а также лейблы собраны в подсказке в колонке деталей.
- **`v4-7410`** 🟡 Ограничение доступа к переопределениям оповещений по командам Grafana: при включении пользователь работает только с переопределениями своих команд. [Документация](https://docs.gmonit.ru/#/modules/alert-pack/setup?id=Ограничение-переопределений-по-командам).
- **`v4-7381`** 🔧 Карта сервисов: исправлены искажённые имена связей и узлов.
- **`v4-7366`** 🔧 APM/Инфраструктура: сопоставление хостов теперь регистронезависимое — метрики APM и инфраструктуры по одному хосту с разным регистром имени больше не расходятся.
- **`v4-7353`** 🚀 Таблицы инфраструктурного мониторинга партиционированы по дням: запросы за период читают только нужные партиции, а очистка по TTL удаляет партиции целиком, без перезаписи данных.

### Июнь 2026

- **`v4-7314`** 🎨 Менеджер инцидентов: на таймлайне assertions соседние оповещения группируются в бакеты — плотные серии больше не сливаются в сплошную полосу, каждая группа читается отдельно.
- **`v4-7311`** 🚀 Менеджер инцидентов: выборка выбросов на странице инцидента и на таймлайне assertions ограничена самыми сильными отклонениями (лимит задаётся `OUTLIERS_MAX_ROWS`) — инциденты с тысячами выбросов больше не тормозят открытие страницы.
- **`v4-7307`** 🔧 Менеджер инцидентов: исправлено субгруппирование метрик на таймлайне assertions.
- **`v4-7284`** 🎨 Переработана форма переопределения оповещений.
- **`v4-7283`** 🟡 Добавлено оповещение об истёкшей лицензии GMONIT: помимо предупреждения за 15 дней теперь срабатывает отдельный alert в момент истечения; в UI-баннере появилось уведомление, что новые данные от агентов не поступают до продления.
- **`v4-7282`** 🔧 Обновлена базовая версия Grafana до 12.4.5 с обновлением зависимостей коллектора.
- **`v4-7261`** 🟡 1С: добавлены страницы журнала регистрации и технического журнала с гибкими фильтрами; переработаны страницы кластера (БД, ошибки, входящие вызовы, блокировки, сессии) и сводка на главной — глубокий анализ событий и нагрузки доступен прямо из GMonit, без выгрузок и сторонних инструментов.
- **`v4-7258`** 🔧 APM: исправлено имя запроса на панели частоты ошибок по транзакции.
- **`v4-7248`** 🟡 MCP-сервер: интегрирован skill — встроенные плейбуки расследования (RCA по оповещению и симптому), справочник таблиц, UDF и шаблонные SQL-запросы. AI-агенты теперь работают с GMonit предсказуемее и без выдумывания структур.
- **`v4-7243`** 🔧 Менеджер инцидентов: исправлен расчёт квантилей для метрик с пропусками — на разреженных графиках значения больше не «улетают».
- **`v4-7234`** 🔧 Исправлен полноэкранный режим панелей при переключении между табами.
- **`v4-7233`** 🟡 APM: с карты сервисов теперь можно перейти на детальную страницу внешнего HTTP-сервиса — со списком вызывающих транзакций, частотой обращений и временем ответа. Легче находить, какие транзакции зависят от тормозящего внешнего сервиса.
- **`v4-7216`** 🔧 Менеджер инцидентов: исправления отображения labels на таймлайне assertions.
- **`v4-7206`** 🟡 Релиз коробочных оповещений GMONIT и страницы их настройки. Оповещения поставляются с шаблонами и порогами по умолчанию; пороги переопределяются прямо из интерфейса — на уровне приложения, хоста, контейнера или БД. Поддержана эскалация по матрице RACI — для каждого правила задаются ответственные роли и уровни. Включение и интервал срабатывания управляются переменными окружения `GMONIT_GRAFANA_DEFAULT_ALERT_PACK_ENABLED` (по умолчанию `false`) и `GMONIT_GRAFANA_DEFAULT_ALERT_PACK_INTERVAL` (по умолчанию `1m`).
- **`v4-7192`** 🎨 Состояние сайдбара (свёрнут/развёрнут) сохраняется между сессиями.
- **`v4-7169`** 🔧 Исправлена работа выпадающих фильтров на нескольких страницах.
- **`v4-7164`** 🎨 Добавлен фильтр окружения на страницы списка браузерных приложений, контейнеров и хостов.
- **`v4-7164`** 🔧 Мобильный мониторинг: исправлена группировка URL со специальными символами.
- **`v4-7124`** 🟡 Менеджер инцидентов: на таймлайне аномалий плотные группы выбросов раскрываются по клику — можно посмотреть каждый отдельный интервал отклонения, не теряя обзора. Лимит выборки задаётся переменной `OUTLIERS_MAX_ROWS` (по умолчанию `1000`) и применяется как к группированному виду, так и к детальному.

### Май 2026

- **`v4-7057`** 🎨 Менеджер инцидентов: на таймлайне событий инцидента добавлены фильтры по типу (оповещения, аномалии, релизы, переподключения), переоформлены сами события — стало проще находить нужное.
- **`v4-7051`** 🟡 Менеджер инцидентов: добавлен режим Materialized Outliers — обнаружение выбросов вынесено в фоновый процесс на материализованные представления, страница инцидента и таймлайн открываются заметно быстрее. Включается флагом `MATERIALIZED_OUTLIERS_ENABLED`, [инструкция по включению и бэкфилу](https://docs.gmonit.ru/#/modules/incidents/setup?id=materialized-outliers).
- **`v4-7048`** 🎨 Пользовательские дашборды: список переработан — добавлена навигация по папкам и улучшен поиск.
- **`v4-7033`** 🚀 APM: ускорены страницы ошибок, веб- и backend-транзакций.
- **`v4-7027`** 🎨 Менеджер инцидентов: улучшено отображение значений в таблицах оповещений и инцидентов.
- **`v4-7003`** 🎨 Редизайн страницы трассировки транзакции: спаны и атрибуты разнесены по табам, добавлена сводная таблица параметров — детали трассы стали понятнее.
- **`v4-6992`** 🟢 OracleDB: добавлена вкладка «Активность» с топом SQL по затраченному времени, активными сессиями и блокировками. Полезно для диагностики проблем в Oracle напрямую из GMONIT.
- **`v4-6991`** 🔧 APM/Масштабируемость: исправлен рендер xy-панелей зависимости CPU, БД и времени отклика от пропускной способности.
- **`v4-6980`** 🔧 Обновлена базовая версия Grafana до 12.4.3 с патчем безопасности.
- **`v4-6972`** 🟡 Менеджер инцидентов: устаревшие или ложные инциденты теперь можно удалять вручную, а инциденты из разных окружений больше не смешиваются между собой.
- **`v4-6953`** 🔧 Менеджер инцидентов: фильтр окружения теперь применяется и к таблице инцидентов.
- **`v4-6950`** 🔧 Менеджер инцидентов: правки отрисовки в панели «Оповещения и аномалии».
- **`v4-6943`** 🟡 Добавлен MCP-сервер коллектора. Теперь к GMONIT можно подключить AI-агента (Claude, Codex и другие) и спрашивать про здоровье приложений, схему таблиц и состояние pipeline на естественном языке — не открывая UI.
- **`v4-6890`** 🔧 Менеджер инцидентов: хотфиксы конфигурации ML-сервиса и фильтра окружения.
- **`v4-6878`** 🎨 Пользовательские дашборды: в хлебных крошках отображается родительская папка.
- **`v4-6876`** 🎨 Главная страница: stat-панели сортируются по убыванию значения — самые проблемные приложения сверху.
- **`v4-6865`** 🔧 Менеджер инцидентов: при ошибках конфигурации ML-сервиса показывается понятное сообщение вместо общего сбоя.
- **`v4-6857`** 🎨 Менеджер инцидентов: фильтр окружения поддерживает выбор сразу нескольких значений.

### Апрель 2026

- **`v4-6837`** 🎨 Пользовательские дашборды: поддержка `relativeTimeRange` и `timeShift` на уровне панели — у конкретной панели может быть свой относительный диапазон или сдвиг во времени, независимо от диапазона дашборда.
- **`v4-6832`** 🎨 Менеджер инцидентов: добавлен фильтр по окружению на таймлайне и панели выбросов.
- **`v4-6828`** 🟡 На главную добавлен список активных оповещений — теперь его видно сразу при заходе, без перехода в Alerting Grafana.
- **`v4-6822`** 🔧 Менеджер инцидентов: исправлен errors-barchart в карточке инцидента.
- **`v4-6820`** 🎨 APM/Базы данных: ускорена страница и добавлена панель Top DB statements by consumed time.
- **`v4-6806`** 🔧 APM: исправлена обработка `connect` для Django-агента.
- **`v4-6798`** 🔧 1С: исправление в выявлении подозреваемых причин таймаутов кластера.
- **`v4-6793`** 🎨 Пользовательские дашборды: поддержка ссылок (panel links) на панелях.
- **`v4-6772`** 🎨 Менеджер инцидентов: расширения панели «Оповещения и аномалии» в карточке инцидента.
- **`v4-6743`** 🔧 APM: исправление в обновлении agent-id при переподключении агента.
- **`v4-6741`** 🎨 Полностью переработаны формы SLO и пользовательских дашбордов; добавлена таблица SLO-алертов прямо в карточке SLO — больше не нужно ходить в Grafana, чтобы посмотреть, что срабатывает.
- **`v4-6730`** 🎨 Менеджер инцидентов: в карточку инцидента добавлена панель «Оповещения и аномалии».
- **`v4-6728`** 🟡 Добавлены правила переименования транзакций. Теперь можно задать на уровне приложения, как сворачивать имена транзакций — полезно для приложений с динамическими URL в имени. [Документация](https://docs.gmonit.ru/#/modules/settings/guide?id=настроить-правила-переименования-и-исключения-транзакций).
...
* **`v4-5821`** 🆕 Поддержан сбор дополнительной метаинформации инфраструктурного мониторинга. Теперь можно получать огромное кол-во справочной информации про наблюдаемые системы.
