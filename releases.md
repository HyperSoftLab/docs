...
- Добавлен браузерный GeoIP, который позволяет определять город местоположения пользователя по его IP-адресу. 
  Для работы требуется файл базы данных GeoIP в формате MMDB. Для настройки добавьте переменную окружения `GEOIP_DB_PATH`. Пример конфигурации:
  ```yaml
  collector: 
    environment:
