...
При необходимости иметь функцию проверки уязвимостей в контуре без доступа к интернету, можно указать путь
и параметры подключения к (локальной) базе данных уязвимостей. Для этого её нужно создать и подключить:

0) (На любой машине) Должна быть установлена Java 11+.
1) Скачать нужное ПО: https://github.com/dependency-check/DependencyCheck/releases/tag/v12.1.1
2) Распаковать.
3) Выполнить команду.
   При этом вам необходимо придумать имя пользователя и пароль для файла базы данных. Эти учётные данные необходимы, чтобы защитить доступ к файлу с базой данных уязвимостей.
   (Если возвращается ошибка 403 или 404, просто попробуйте ещё раз. Также бывает что первый сгенерированный ключ не работает, в этом случае просто сгенерируйте новый.)
   - Windows: `РАСПАКОВАННЫЙ_АРХИВ\dependency-check\bin\dependency-check.bat --updateonly --nvdApiKey ЗДЕСЬ_КЛЮЧ_ДО_БАЗЫ_УЯЗВИМОСТЕЙ_NVD. --dbUser ИМЯ_ПОЛЬЗОВАТЕЛЯ --dbPassword ПАРОЛЬ`
   - Linux:   `РАСПАКОВАННЫЙ_АРХИВ/dependency-check/bin/dependency-check.sh  --updateonly --nvdApiKey ЗДЕСЬ_КЛЮЧ_ДО_БАЗЫ_УЯЗВИМОСТЕЙ_NVD. --dbUser ИМЯ_ПОЛЬЗОВАТЕЛЯ --dbPassword ПАРОЛЬ`
5) Дождаться выполнения. Примерно 5 минут.
6) База уязвимостей готова по пути: `РАСПАКОВАННЫЙ_АРХИВ/dependency-check/data/odc.mv.db`
7) Скопировать этот файл на хост с GMonit Collector (или настроить volume в Docker).
8) В числе прочих, выставить следующие параметры (имя пользователя и пароль - из пункта 3):
