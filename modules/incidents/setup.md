...
## Разбор первопричины через LLM

Коллектор может отправлять состав инцидента в языковую модель и сохранять её заключение в карточку — краткое описание произошедшего и ранжированный список вероятных причин.

<!-- STEP_GUIDE:START Подключение разбора первопричины инцидента через языковую модель: переменные окружения, требования к API модели, момент запуска разбора, где виден результат, поведение при ошибке и при выключенной возможности. -->

Возможность выключена по умолчанию. Для включения задайте переменные окружения коллектора и перезапустите его:

| Переменная | Значение |
|---|---|
| `INCIDENT_LLM_RCA_ENABLED` | `true` |
| `LLM_RCA_API_URL` | Базовый адрес API, например `https://ai.api.example.net` |
| `LLM_RCA_API_KEY` | Токен доступа к API |
| `LLM_RCA_MODEL` | Идентификатор модели, например `gpt://<model-uri>` |
| `LLM_RCA_TIMEOUT_MS` | Таймаут запроса к модели в миллисекундах. По умолчанию `60000` |

Требования к API:

- Модель должна быть доступна по **OpenAI-совместимому API** (метод chat completions).
- В `LLM_RCA_API_URL` указывается только базовый адрес — путь к методу коллектор добавляет сам.
- Токен передаётся в заголовке `Authorization`. Если API не требует авторизации, `LLM_RCA_API_KEY` можно не задавать.

> Если `INCIDENT_LLM_RCA_ENABLED=true`, а адрес модели не задан, коллектор не запустится. Включайте флаг вместе с остальными переменными.

### Что уходит в модель

Данные передаются **как есть, без обезличивания**. В запрос попадают:

- период инцидента и число затронутых хостов;
- имена приложений;
- имена сработавших алертов, связанные с ними метрики и длительность;
- отклонившиеся метрики: приложение, имя метрики, имя хоста, значение и границы нормы.

Не передаются: содержимое запросов и ответов, логи, тексты SQL, параметры транзакций и любые пользовательские данные.

Тем не менее имена приложений, хостов и метрик — это сведения о вашей инфраструктуре. **Подключайте только те модели и тех поставщиков, которым доверяете**, и учитывайте требования вашей политики обработки данных: развёрнутая в контуре модель предпочтительнее внешнего сервиса.

Обезличивание данных перед отправкой планируется в следующих версиях.

### Как это работает

1. При обнаружении инцидента коллектор сохраняет заготовку разбора и ставит задачу в фоновую очередь — детектирование не ждёт ответа модели.
2. В модель уходит состав инцидента: затронутые сервисы, сработавшие алерты и отклонившиеся метрики.
3. Ответ сохраняется и появляется в [карточке инцидента](/modules/incidents/guide.md#Просмотреть-карточку-инцидента): описание и вероятные причины по убыванию правдоподобия.

Разбор выполняется для инцидентов, обнаруженных **после** включения возможности. Для более старых инцидентов его можно запросить отдельно — через [AI-агента по MCP](/modules/mcp/guide.md#Инциденты).

Если запрос к модели не удался, в карточке сохраняется отметка об ошибке с её причиной — инцидент при этом остаётся полноценным, без разбора.

Пока возможность выключена, инциденты формируются и обогащаются как обычно: разбор — дополнение к карточке, а не условие её появления.
